...
description: "Generate an English business-rules Markdown document from Spanish or multilingual Spark SQL, Databricks SQL, or JSON table metadata. Use when extracting table descriptions, field semantics, table relationships, proposed business metrics, business restrictions, data-quality rules, or testing scenarios. Writes the result to out/business_rules/business_rules.md."
---
...
- If a file path is supplied, read that exact file; do not substitute an excerpt,
  attachment, or previously generated report.
- Extract catalog, schema, table, table description, columns, data types,
  ...
  hints.
- For SQL, parse `CREATE TABLE` definitions and `ALTER TABLE` constraint
  statements independently. Foreign keys are commonly declared after the table
  definition, so searching only inside `CREATE TABLE` blocks is insufficient.
...
- Compute table, primary-key, and foreign-key counts from the current input. Do
  not copy counts from a file header or invent expected counts. If a header count
  differs from the parsed count, report both values and flag the discrepancy.
...
- In SQL, preserve every explicit `ALTER TABLE ... FOREIGN KEY ... REFERENCES`
  declaration as one relationship row. For each row retain the exact source
  table, source column(s), target table, target column(s), and constraint name
  when available.
- Before writing the report, compare the number of relationship rows with the
  number of explicit foreign-key declarations. If they differ, stop and repair
  the extraction or document the exact omissions; do not claim complete
...
Return only the complete contents of `business_rules.md` in English. Use these
exact headings, without numeric prefixes, and keep them in this order:
...
## Business Restrictions and Data-Quality Rules
## Cross-Table Test Scenarios
...
- Make the document useful even when only a subset of the full schema is
  supplied.
- Include a source-metadata summary with the actual parsed counts for tables,
  primary keys, and foreign keys. Never hardcode counts from an earlier run.
...
8. All ten required headings appear exactly as specified, without numeric
  prefixes or alternate spellings.
9. The output is valid, self-contained Markdown and contains no commentary
...
Before reporting completion, validate that the output file exists, is non-empty,
contains all ten exact headings, contains the input and output paths, and
accounts for every explicit foreign key found in the input. If validation cannot
be executed, state that clearly instead of claiming success.
